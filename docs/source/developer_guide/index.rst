...
``aiida-charmm-gui`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-charmm-gui`` repository (preferably using ``aiida-charmm-gui`` as the project name)

The documentation is now available at `aiida-charmm-gui.readthedocs.io <http://aiida-charmm-gui.readthedocs.io/>`_.
